//
//  VAICustomRecognitonViewController.m
//  VAIClient
//
...
@interface VAICustomRecognitonViewController ()
...
@implementation VAICustomRecognitonViewController
...
- (void)loadView 
{
	UIView *tmpView = [[UIView alloc] initWithFrame:[[VAIClientUIManager sharedInstance] VRBackgroundFrame]];
...
- (void)viewDidLoad 
{
    ...
    if ([VAISConfig sharedInstance].playStartMusicSwitch)
    {
        [self createInitView];
    }
    else 
...
- (void)VoiceRecognitionClientWorkStatus:(int)aStatus obj:(id)aObj
{
    switch (aStatus)
    {
        ...
        case EVoiceRecognitionClientWorkStatusFinish: // 识别正常完成并获得结果
        {
            ...
            {
                NSString *tmpString = [[VAISConfig sharedInstance] composeInputModeResult:aObj];
        ...
        {
            if ([VAISConfig sharedInstance].playStartMusicSwitch) // 如果播放了提示音，此时再给用户提示可以说话
            {
                [self createRecordView];
            }
            
            if ([VAISConfig sharedInstance].voiceLevelMeter)  // 开启语音音量监听
            {
...
- (void)createInitView
{
    ...
    tmpImageView.center = [[VAIClientUIManager sharedInstance] VRRecordBackgroundCenter];
    [_dialog addSubview:tmpImageView];
    [tmpImageView release];
    
    UILabel *tmpLabel = [[UILabel alloc] initWithFrame:[[VAIClientUIManager sharedInstance] VRRecordTintWordFrame]];
    ...
    tmpLabel.center = [[VAIClientUIManager sharedInstance] VRTintWordCenter];
    [_dialog addSubview:tmpLabel];
    [tmpLabel release];
    
    UIButton *tmpButton = [UIButton buttonWithType:UIButtonTypeCustom];
    tmpButton.frame = [[VAIClientUIManager sharedInstance] VRCenterButtonFrame];
    tmpButton.center = [[VAIClientUIManager sharedInstance] VRCenterButtonCenter];
...
- (void)createRecordView
{
    ...
    UIButton *tmpButton = [UIButton buttonWithType:UIButtonTypeCustom];
    tmpButton.frame = [[VAIClientUIManager sharedInstance] VRLeftButtonFrame];
    tmpButton.backgroundColor = [UIColor clearColor];
    [tmpButton setTitle:NSLocalizedString(@"StringCancel", nil) forState:UIControlStateNormal];
    tmpButton.titleLabel.font = [UIFont boldSystemFontOfSize:20.0f];
    tmpButton.titleLabel.textColor = [UIColor whiteColor];
    ...
    tmpButton.frame = [[VAIClientUIManager sharedInstance] VRRightButtonFrame];
...
- (void)createRecognitionView
{
	...
	tmpImageView.center = [[VAIClientUIManager sharedInstance] VRRecognizeBackgroundCenter];
	[_dialog addSubview:tmpImageView];
	[tmpImageView release];
    
    UILabel *tmpLabel = [[UILabel alloc] initWithFrame:[[VAIClientUIManager sharedInstance] VRRecognizeTintWordFrame]];
    tmpLabel.backgroundColor = [UIColor clearColor];
    tmpLabel.font = [UIFont boldSystemFontOfSize:28.0f];
    tmpLabel.textColor = [UIColor whiteColor];
